...
class CustomEvalCallback(pl.Callback):
    """Custom callback that runs every 1000 steps"""
    
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if trainer.global_step % 2000 == 1 and trainer.global_step > 1:
            print(f"Evaluating at step {trainer.global_step-1}")
            os.makedirs(f"competition/state_sm_tahoe/eval_{trainer.global_step-1}", exist_ok=True)
                
            subprocess.run([
                "uv", "run", "state", "tx", "infer",
                "--output", f"competition/state_sm_tahoe/eval_{trainer.global_step-1}/prediction_{trainer.global_step-1}.h5ad",
                "--model_dir", "competition/state_sm_tahoe", 
                "--checkpoint", f"competition/state_sm_tahoe/checkpoints/step={trainer.global_step-1}.ckpt",
            ...
            subprocess.run([
                "cell-eval", "run",
                "-ap", f"competition/state_sm_tahoe/eval_{trainer.global_step-1}/prediction_{trainer.global_step-1}.h5ad",
                "-ar", "competition_support_set/hepg2.h5",
                "--num-threads", "64",
                "--profile", "vcc",
                "-o", f"competition/state_sm_tahoe/eval_{trainer.global_step-1}"
            ])
...
def run_tx_train(cfg: DictConfig):
    ...
    # Add custom callback that runs every 1000 steps
    custom_eval_callback = CustomEvalCallback()
    callbacks = ckpt_callbacks + [batch_speed_monitor, custom_eval_callback]
